...
public class PornHubBuilder extends ContainerResourceBuilder {

    @Override
    public boolean canParse() {
        String sURL = mURL.toExternalForm();

        if (sURL.matches(".*pornhub.com/video/show/title/.*")) {
    ...
    public String getDownloadURL(Container container) {
        ...
        Elements flowPlayerData = document.select("div.play_video_link > div > a");
        if (flowPlayerData.size() > 0) {
            getTitleFromHtml(document);
            Element playerTag = flowPlayerData.get(0);
            Log.d("Link " + playerTag.attr("href"));
            return playerTag.attr("href");
        }
        return null;
    }

    private void getTitleFromHtml(Document document) {
        Elements title = document.select(".title>div>h1");
        if (title.size() > 0) {
            mTitle = title.text();
        }
    }
